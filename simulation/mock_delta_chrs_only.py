...
def parse_agp(query_agp, ref_chrom_pattern):
    ...
    with open(query_agp) as f:
        for line in f:
            ...
            if cols[4] == 'W':
                ...
                if re.match(ref_chrom_pattern, ref_chrom):
                    ref_chrom_set.add(ref_chrom)
            elif cols[4] in {'U', 'N'}:
    ...
    for scaffold_name, alignments in alignments_dict.items():
        # retain scaffolds containing more than 2 contigs (alignments)
        if len(alignments[1:]) >= 2:
            scaffold_set.add(scaffold_name)

    for ref_chrom, ctg_list in ref_chrom_dict.items():
        ctg_list.sort(key=lambda x: x[-1])
    
    return alignments_dict, ref_chrom_dict, scaffold_set, ref_chrom_set


def parse_fasta(fasta, id_set):
...
def mock_delta_file(alignments_dict, ref_chrom_dict, ref_seq_len_dict, query_seq_len_dict, scaffold_set, ref_chrom_set, ref_fasta_chrs, query_fasta_chrs):

    
    ctg_pos_dict = dict()
    for ref_chrom, ctg_list in ref_chrom_dict.items():
        if ref_chrom not in ref_chrom_set:
            continue
        start = 1
    ...
    with open(delta_file, 'w') as f:
        ...
        for scaffold_name, alignments in alignments_dict.items():
            if scaffold_name not in scaffold_set:
                continue
            last_scaffold_end = 0
            last_ctg_end = 0
            last_ref_chrom = ''
            ...
            for scaffold_start, scaffold_end, ctg_name, orient in alignments[1:]:
                if ctg_name.rsplit('_', 2)[0] not in ref_chrom_set:
                    continue
...
def main():
    ...
    parser.add_argument('ref_chrom_pattern', help='pattern for reference chromosomes')
    args = parser.parse_args()

    alignments_dict, ref_chrom_dict, scaffold_set, ref_chrom_set = parse_agp(args.query_agp, args.ref_chrom_pattern)
    ref_fasta_chrs, ref_seq_len_dict = parse_fasta(args.ref_fasta, ref_chrom_set)
    query_fasta_chrs, query_seq_len_dict = parse_fasta(args.query_fasta, scaffold_set)
    mock_delta_file(alignments_dict, ref_chrom_dict, ref_seq_len_dict, query_seq_len_dict, scaffold_set, ref_chrom_set, ref_fasta_chrs, query_fasta_chrs)
